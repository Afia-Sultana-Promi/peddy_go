function activeBtn(id, category) {
  // Select all buttons with the 'categoryBtn' class
  const allButtons = document.querySelectorAll('.categoryBtn');

  // Toggle 'active' and 'not-active' classes for all buttons
  allButtons.forEach(button => {
    button.classList.toggle('active', button.id === id);
    button.classList.toggle('not-active', button.id !== id);
  });
  ...
  // Use setTimeout to delay the rendering of the category
  setTimeout(() => {
    renderByCategory(category);
  }, 700); // Delay for 700 milliseconds
}
  ...
  const renderCategoryButton = async()=>{
    ...
    arr.forEach((obj)=>{
      div.innerHTML += `
      <button id='${obj.id}' class='categoryBtn w-52 h-16 bg-transparent text-lg px-3 py-5 border rounded-xl flex gap-3 items-center justify-center font-bold transition-all hover:bg[#becfeb] hover:cursor-pointer' onclick='activeBtn("${obj.id}", "${obj.category}")'>
